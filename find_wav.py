...
def find_wav(data_root, scp_dir, scp_name='wpe', wav_type='Far', n_split=1):
    ...
    for j in range(n_split):
        with codecs.open(os.path.join(scp_dir, '{}.{}.scp'.format(scp_name, j+1)), 'w') as handle:
            # print(lines[j])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nj', type=int, default=10, help='number of split files')
    args = parser.parse_args()
    print(args)
    find_wav(data_root=args.data_dir, scp_dir=args.scp_dir, scp_name=args.scp_name, wav_type=args.wav_type,
             n_split=args.nj)
