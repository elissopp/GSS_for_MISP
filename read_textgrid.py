...
def prepare_wav_scp(data_root, store_dir, set_type='train'):
    id_postfix = 'Far'
    all_wav_lines = []
    set_dir = os.path.join(data_root,"audio",set_type,"far")
    for wav_path in os.listdir(set_dir):
        id_prefix = os.path.split(wav_path)[-1].split('_')[:4]
        record_id = '_'.join(id_prefix) + "_" + id_postfix
        record_path = os.path.join(set_dir,record_id + ".wav")
        all_wav_lines.append('{} {}'.format(record_id,record_path))
...
def prepare_segments_text_utt2spk_worker(transcription_path, store_dir, processing_id=None, processing_num=None):
    segments_lines = []
    text_sentence_lines = []
    utt2spk_lines = []
    tier_name = '内容层'
        ...
        if processing_token:
            ...
            for speaker in speaker_list:
                tg = read_textgrid_from_file(filepath=os.path.join(
                    transcription_path, '{}_{}_{}_{}_Near_{}.TextGrid'.format(
...
def prepare_segments_text_utt2spk_manager(data_root, set_type, store_dir, processing_num=1):
    transcription_path = os.path.join(data_root,"transcription",set_type)
    if processing_num > 1:
        ...
        for i in range(processing_num):
            part_result = pool.apply_async(prepare_segments_text_utt2spk_worker, kwds={
                'store_dir': store_dir, 'processing_id': i, 'processing_num': processing_num})
        ...
        segments_lines, text_sentence_lines, utt2spk_lines = prepare_segments_text_utt2spk_worker(transcription_path=transcription_path,store_dir=store_dir)
...
if __name__ == '__main__':
    ...
    prepare_wav_scp(data_root=args.data_root, store_dir=args.store_dir, set_type=args.set_type)
    if not args.only_wav:
        print('Preparing segments,text_sentence,utt2spk in {} for {} set'.format(args.store_dir, args.set_type))
        prepare_segments_text_utt2spk_manager(data_root=args.data_root, set_type=args.set_type, store_dir=args.store_dir, processing_num=args.nj)
