...
def wpe_worker(wav_scp, processing_id=None, processing_num=None, data_root='MISP_121h', output_root='MISP_121h_WPE_'):
    sampling_rate = 16000
    iterations = 5
    stft_options = dict(
        size=512,
    ...
    cnt = 0
    for wav_idx in tqdm(range(len(wav_lines)), leave=True, desc='0' if processing_id is None else str(processing_id)):
        ...
        if processing_token:
            ...
            checkpath = wav_list[0].replace(data_root, output_root)
...
def wpe_manager(wav_scp, processing_num=1, data_root='MISP_121h', output_root='MISP_121h_WPE_'):
    if processing_num > 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('data_root', type=str, default='wpe', help='input misp data root')
    parser.add_argument('output_root', type=str, default='wpe', help='output wpe data root')
    ...
    wpe_manager(wav_scp=args.wav_scp, processing_num=args.nj, data_root=args.data_root, output_root=args.output_root)